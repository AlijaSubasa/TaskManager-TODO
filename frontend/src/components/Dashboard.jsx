...
const Dashboard = ({ user, onLogout }) => {
    ...
    // 2. State za formu (dodavanje)
    const [title, setTitle] = useState('');
    const [description, setDescription] = useState('');
    const [categoryId, setCategoryId] = useState(1);

    // 3. State za filtriranje
    const [filterCategory, setFilterCategory] = useState('');
    const [filterStatus, setFilterStatus] = useState('');

    // Funkcija za učitavanje podataka - definisana sa useCallback radi efikasnosti
    ...
    // Učitaj podatke pri montiranju ili promjeni filtera
    useEffect(() => {
        loadData();
    }, [loadData]);
    ...
    const toggleStatus = async (task) => {
        ...
        try {
            ...
            alert("Greška pri ažuriranju statusa. Provjerite bazu.");
        }
    };
                        ...
                        <tr key={task.id} style={{ borderBottom: '1px solid #ddd' }}>
                            ...
                            <td style={{
                                ...
                                color: task.status === 'Završeno' ? 'green' :
                                    task.status === 'U tijeku' ? '#d35400' : 'red',
                                fontWeight: 'bold'
                            }}>
                                {task.status}
                            </td>
                            <td style={{ padding: '10px' }}>
                                <button onClick={() => toggleStatus(task)} style={{ marginRight: '5px', padding: '5px 10px', cursor: 'pointer' }}>
                                    Status
                                </button>
                                <button onClick={() => handleDelete(task.id)} style={{ background: '#dc3545', color: 'white', border: 'none', padding: '5px 10px', cursor: 'pointer', borderRadius: '3px' }}>
                                    Obriši
                                </button>
                            </td>
                        </tr>
